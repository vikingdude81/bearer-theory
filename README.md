...
The paper proposes a design law with three regimes (idle bearer, difficulty valley,
load-bearing and bearer-specific) and demonstrates the mechanism across three substrates
that share nothing but the loop: a reinforcement-learning forager, a transformer with a
fast-weight memory, and a competitive-fatigue dynamical system. A self versus wrong-bearer
gap of 0.51 (replicated, attention-dissociated, holding at scale) establishes load-bearing
...
pre-linguistic developmental interface, distinguishes an *engineering gap* from a
*metaphysical gap*, and executes a research program with four probe systems and a flagship
construction-order experiment. Constraints present from the first training step are
*internalized* (removable at almost no cost); inference-only overlays are a *tax* and
fine-tuned retrofits a *crutch*. The experiments locate the wall; they do not dissolve it.

---

## Code and experiments

- **Constitutive GPT** (the fast-weight bearer experiments, M1–M4): https://github.com/vikingdude81/constitutive-gpt
- **Gabriel's Horn** (answerability probe): https://github.com/vikingdude81/gabriels-horn
